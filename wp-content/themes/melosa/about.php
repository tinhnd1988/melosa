<?php
/*
/*
/* Template Name: About
/*
*/
get_header();
?>
<div class="pageHeader">
				<div class="call_us">
					<div class="icon_call"><img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/phone.png"></div>
					<ul>
						<li>
							<div class="title_call">CEN Group</div>
							<span>0903.85.99.11</span>
						</li>
						<li>
							<div class="title_call">CTY CPBĐS La Bàn</div>
							<span>0912 961 727</span>
						</li>
						<li>
							<div class="title_call">Sàn GD SunLand SG</div>
							<span>0938 779 660</span>
						</li>
						<li>
							<div class="title_call">CTY CPTV TCF Việt Nam</div>
							<span>0908 49 6446</span>
						</li>
						<li>
							<div class="title_call">CTY TNHH Hưng Thịnh Phú</div>
							<span>0934 839 868</span>
						</li>
						<li>
							<div class="title_call">CTY CPBĐS TECHCOMREAL</div>
							<span>0908 06 06 79</span>
						</li>
						<li>
							<div class="title_call">CTY CPTV Đông Nam Á</div>
							<span>0909 333 447</span>
						</li>
						<li>
							<div class="title_call">CTY CPMG BĐS VÂN HÀ</div>
							<span>0906 684 499</span>
						</li>
					</ul>
				</div>				
				<div class="titleBar">
					<h1>Giới thiệu</h1>
					<ul>
						<li class="ui_tabs"><span class="Tab1">Melosa Garden</span></li>
						<li class="ui_tabs active"><span class="Tab2">Chủ đầu tư</span></li>
						<li class="ui_tabs"><span class="Tab3">Đội ngũ</span></li>
					</ul>
				</div>
			</div>
			<div class="post_content">
				<div id="Tab1" class="content_tab" style="display:none;">
					<div class="container">
						<h4> TẬN HƯỞNG CUỘC SỐNG TINH HOA </h4>
						<p>Những cung bậc bình yên, lắng đọng, nơi con người hòa mình trong ánh sáng, sông nước và thiên nhiên chính là những tinh hoa cuộc sống.</p>
						<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/gioi_thieu_1.png">
					</div>
				</div><!-- end tab1 -->
				<div id="Tab2" class="content_tab">
					<div class="container">
						<div class="leftCol">
							<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/about-leftimg.jpg">
						</div>
						<div class="rightCol">
							<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/khangdien-logo.png">
							<p>
								Với 15 năm kinh nghiệm trong lĩnh vực đầu tư và phát triển bất động sản, Khang Điền tự hào là đơn vị tiên phong cho những dự án nhà có tiền ích được đầu tư cao tại Q.9 như: Chuỗi Mega Village, Mega Ruby, Mega Residence, Hoja Villa và PARCSpring						
							</p>
						</div>
						<div class="clearfix"></div>
					</div>
					<div class="stats">
						<div class="item">
							<span class="value">15</span>
							<span class="unit">năm</span>
							<p class="desc">Kinh nghiệm</p>
						</div>
						<div class="item">
							<span class="value">100</span>
							<span class="unit">ha</span>
							<p class="desc">Quỹ đất sạch</p>
						</div>
						<div class="item">
							<span class="value">17</span>
							<span class="unit">dự án</span>
							<p class="desc">Tập trung tại Q.2 và Q.9</p>
						</div>
						<div class="bottom-text">
							<span>Khang Điền vinh dự được hợp tác cùng những đơn vị hàng đầu Việt Nam và Quốc tế</span>
						</div>										
					</div>
					<div class="sponsor">
						<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/sponsor.jpg">
					</div>
				</div><!-- end tab2 -->
				<div id="Tab3" class="content_tab" style="display:none;">
					<div class="container">
						<h4> TẬN HƯỞNG CUỘC SỐNG TINH HOA </h4>
						<p>Những cung bậc bình yên, lắng đọng, nơi con người hòa mình trong ánh sáng, sông nước và thiên nhiên chính là những tinh hoa cuộc sống.</p>
						<img src="<?php echo esc_url( get_template_directory_uri() ); ?>/images/gioi_thieu_1.png">
					</div>
				</div><!-- end tab3 -->
			</div>
<?php get_footer(); ?>
